 ...
 *   legend: an array of legend to appear in the legend, one for each wedge
 *   lx, ly: the upper-left corner of the chart legend
 * Returns: 
 *    An <svg> element that holds the pie chart.
 *    The caller must insert the returned element into the document.
 */
function pieChart({data, width, height, cx, cy, r, color, legend, lx, ly}) {
    ...
    for(var i = 0; i < data.length; i++) {
        ...
        path.setAttribute("fill", color[i]);   // Set wedge color
        ...
        icon.setAttribute("fill", color[i]);   // Same fill color as wedge
